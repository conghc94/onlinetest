        ...
        <script type="text/javascript" language="javascript">
            function getConfirmation(masv) {
                if(confirm("Xóa kết quả thi có id = " + masv + " khỏi danh sách?"))
                    window.location = ("deletestudent.php?id=" + masv);
            }
    ...
    <body onload="document.searchBar.focus.focus()">
        ...
        <div id="main_content">
            ...
            <div class="search-wrapper">
                <form name="searchBar" action="search.php" onsubmit="" >
        ...
        $sql = "SELECT * FROM `ketquathi` where `hoten` LIKE '%".$_GET['focus']."' (SELECT * FROM `ketquathi` WHERE `madethi` IN (SELECT `madethi` FROM `dethi` WHERE `magv` = '".$magv."'))";
        $sql = "SELECT * FROM `ketquathi`";
